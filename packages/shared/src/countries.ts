...
/**
 * Liste complète des 249 codes ISO 3166-1 alpha-2, avec leur nom usuel en français.
 * Donnée statique volontairement embarquée plutôt qu'une dépendance externe (voir
 * docs/DECISIONS.md) : liste stable, aucune API `Intl.DisplayNames` requise (compatibilité
 * Hermes/React Native non garantie sur toutes les versions), pas de dépendance à maintenir.
 * Triée par code ISO.
 */
export const ISO_COUNTRIES: readonly CountryOption[] = [
...
const COUNTRY_NAME_BY_CODE: ReadonlyMap<string, string> = new Map(
  ISO_COUNTRIES.map((c) => [c.code, c.name]),
);

export function isValidCountryCode(code: string): boolean {
  return COUNTRY_NAME_BY_CODE.has(code);
}

/** Nom français du code ISO, ou `undefined` s'il est inconnu. */
export function getCountryName(code: string): string | undefined {
  return COUNTRY_NAME_BY_CODE.get(code);
}
