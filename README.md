...
- une API NestJS complète : authentification (Argon2 + JWT access/refresh révocable, rate limiting dédié sur les routes d'authentification), households (membres, rôles, invitations par email via Mailpit), catégories système et personnalisées, items (propriétaires multiples, recherche, filtres, pagination, archivage/restauration), uploads d'images (validation réelle du type de fichier), statistiques, exports JSON/CSV ;
...
- documentation Swagger complète (`/api/v1/docs`, `@ApiOperation`/`@ApiResponse` sur toutes les routes) et contrat OpenAPI figé (`docs/openapi.json`) ;
- logs structurés (JSON en production, lisibles en développement), logs d'accès corrélés par requestId, healthcheck de disponibilité (`/health/ready`) vérifiant réellement la base de données ;
...
- Limite explicite de taille des corps JSON/urlencoded (1 Mo) ; les uploads d'images gardent leur propre limite dédiée (10 Mo, `FileInterceptor`).
- Rate limiting (`@nestjs/throttler`) : 100 req/min par défaut sur toute l'API, 10 req/min sur `/auth/register`, `/auth/login` et `/auth/refresh` (cibles privilégiées du bourrage d'identifiants) — testé en e2e.
