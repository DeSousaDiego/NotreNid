...
- **Polish de l'accueil (post-audit)** : en-tête « Notre Nid » + pastille sauge avec le nom du foyer (`useHousehold().households`, aucune requête) + message de bienvenue ; stats Livre/CD/DVD en trois tuiles teintées (nouveaux tokens `tintSage`/`tintPeach`/`tintHoney`, mapping `getCategoryTint`), total intégré au titre (« 12 trésors dans votre nid ») ; état vide dédié (surface pêche, CTA orange « Ajouter un objet » → `/(app)/add-item/category`) quand `totalActiveItems === 0` ; bouton « Voir la collection » conservé, en vert forêt. Robustesse : `data` testé avant `isError` (un refetch en échec conserve le contenu en cache), récents découplés des stats, section récents retirée si vide ; skeletons alignés sur le contenu réel (`RecentItemRowSkeleton`, tuiles à hauteur fixe). `RecentItemRow` : label accessible enrichi, badge catégorie masqué quand la couverture de repli montre déjà l'illustration. Tests : `index.test.tsx` étendu (18 cas), nouveaux `RecentItemRow.test.tsx` et `relativeDate.test.ts`. Connu, préexistant : lancée seule, une suite qui rend `LoadingSkeleton` (Accueil, Collection) passe mais Jest ne rend pas la main (handle ouvert) — sans effet sur la suite complète.
- **Robustesse du flow Ajout/Édition (Lot 1, mobile uniquement) — ✅ validé localement** (lint/typecheck/format verts ; suites ciblées item-form + add-item 139/139 ; suite mobile complète 423/423 en parallèle ET en `--runInBand` ; contrôles par mutation : retirer le verrou, le blocage d'upload ou le désarmement de la garde fait échouer les tests correspondants). Les 1 à 4 échecs variables observés lors de la première session (`join`, `BottomSheet`, `Select`, `CountrySelect`, une fois `ItemFormScreen`) passent tous isolément et ne se sont pas reproduits sur les deux runs complets suivants — contention sous charge machine, non liée au lot : l'état d'envoi de la couverture remonte de `CoverPickerField` à `ItemFormScreen` (`onUploadingChange`) — CTA final et « Précédent » désactivés, retour système ignoré et message dans le footer pendant l'upload ; verrou synchrone `submitLockRef` contre la double soumission (création et édition), relâché après un échec ; erreur de soumission ancrée au footer (au-dessus des boutons, rôle `alert`), effacée au début de chaque tentative ; message de changement de catégorie exact (« Les informations déjà saisies seront perdues. ») ; le code scanné est écrit dans le brouillon dès la lecture (conservé sur `no_match`/`provider_error`/erreur réseau, affiché « Code lu : … ») ; retour arrière via `usePreventRemove` (`expo-router/react-navigation`, aucune dépendance ajoutée — la native stack bloque aussi le geste iOS tant que la garde est active) : étape N → N-1, puis en édition confirmation « Quitter sans enregistrer ? » si `formState.isDirty` ; navigations de succès et sortie confirmée exécutées dans un effet après désarmement de la garde (`exitIntent`), jamais ré-interceptées. Limites connues : non vérifié sur appareil réel (comportement natif du geste iOS / bouton retour à confirmer via OTA) ; `uploadedId` reste local à `CoverPickerField` — quitter l'étape 3 après un upload puis y revenir empêche « Retirer » de supprimer le fichier distant, et un remplacement n'efface pas l'image précédente (fichiers orphelins, sans perte côté utilisateur) ; quitter entièrement le flow de création depuis l'écran Catégorie efface toujours le brouillon sans confirmation (comportement documenté) ; l'effacement des champs facultatifs en édition reste ignoré par l'API (Lot 2, backend).
...
| Exécution réelle de la CI GitHub Actions (push sur `main`, 3 itérations : `.github/workflows/ci.yml` non modifié, puis correctif « Set up pnpm », puis correctif ordre « Build ») | ✅ verte sur le commit `43e059b` (`conclusion: success`, confirmé via l'API GitHub) — premier passage vert de la CI sur GitHub pour ce dépôt, après correction de deux bugs préexistants (voir « Problèmes rencontrés »). |
...
- **CI GitHub Actions jamais verte sur les 5 exécutions précédentes** (y compris celle du commit ayant introduit le workflow en Phase 4 — voir Phase 4 ci-dessous) : la CI ayant réellement tourné sur GitHub pour la première fois en Phase 5, deux bugs préexistants — sans rapport avec le contenu de cette phase — ont été découverts et corrigés dans la foulée :
  1. `pnpm/action-setup@v4` échouait dès la première étape (« Set up pnpm ») avec `Multiple versions of pnpm specified`, car `.github/workflows/ci.yml` fixait `version: 11` en plus du `packageManager: pnpm@11.17.0` déjà déclaré dans `package.json` — l'action refuse la double source de vérité. Corrigé en retirant l'entrée `version:` du workflow.
  2. Une fois ce premier blocage levé, l'étape « Lint » échouait à son tour (`import/no-unresolved` sur `@notre-nid/shared`/`@notre-nid/api-client` dans `apps/mobile`) : ces deux packages sont résolus via leurs champs `package.json` `main`/`types`, qui pointent vers `dist/`, jamais construit avant l'étape « Build », placée trop tard dans le pipeline (après Lint/Typecheck/Test). Un environnement de développement local ne révèle jamais ce problème car `dist/` y reste construit d'une exécution à l'autre. Reproduit localement en supprimant `dist/` puis en relançant `pnpm lint` (mêmes 8 erreurs `import/no-unresolved` que celles vues en CI) ; corrigé en déplaçant l'étape « Build » immédiatement après la génération/validation Prisma, avant Format check/Lint/Typecheck/Test — revérifié localement (lint redevient propre après un `pnpm run build` frais).
  Après ces deux correctifs, l'exécution CI sur le commit `43e059b` s'est terminée avec `conclusion: success` — premier passage vert de la CI sur GitHub pour ce dépôt, confirmé via l'API GitHub (`GET /repos/.../actions/runs`).
